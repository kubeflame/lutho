<script lang="ts">
  ...
  import { tabs, routeString, SecretV1GVRK } from "../lib/util";
  import { KubeDataOpType, TabIndex } from "../lib/types";
  import type { V1Secret } from "@kubernetes/client-node/dist/gen/api";
...
<div class="router-page" in:fade|global={{ duration: 250 }}>
  ...
  {:else if activeTab === TabIndex.YAML}
    <CodeMirror doc={toYaml(secretData)} bind:codeMirrorChanged bind:docStore />
  {/if}
</div>
