<script lang="ts">
  ...
  import { randomUUID, ReplicaSetV1GVRK, routeString } from "../lib/util";
  import type { V1ReplicaSetList } from "@kubernetes/client-node";
  ...
  let replicaSetListData: V1ReplicaSetList;

  const { sockError, isLoading, dataSend, dataList, dataDelete } =
    socketStore();

  $: toolbarContent = [{ index: 0, name: "ReplicaSet List" }];
...
<RouterPage bind:error={$sockError} bind:loading={$isLoading}>
  ...
  <ListTable
    hrefRoot={routeString.replicaSetList}
    isNamespaced={true}
    tableHead={["Name", "Namespace", "Created At", ""]}
    items={replicaSetListData?.items}
