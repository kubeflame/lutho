...
<div class="router-page" in:fade|global={{ duration: 250 }}>
  ...
  {:else if $isLoading}
    <LoadingNewton />
  {:else if persistentVolumeClaimListData}
    <table class="table table-pin-rows">
      ...
      <tbody>
        {#each persistentVolumeClaimListData.items as persistentVolumeClaim}
          <tr>
            <td>
              <a
                class="hover:text-primary"
                href="{routeString.persistentVolumeClaimList}/{persistentVolumeClaim
                  .metadata?.namespace}/{persistentVolumeClaim.metadata?.name}"
                use:link
              >
