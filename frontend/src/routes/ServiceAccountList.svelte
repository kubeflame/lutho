<script lang="ts">
  ...
  import type { V1ServiceAccountList } from "@kubernetes/client-node/dist/gen/api";
...
<div class="router-page" in:fade|global={{ duration: 250 }}>
  ...
  {:else if $isLoading}
    <LoadingNewton />
  {:else if serviceAccountListData}
    <table class="table table-pin-rows">
      ...
      <tbody>
        {#each serviceAccountListData.items as serviceAccount}
          <tr>
            <td>
              <a
                class="hover:text-primary"
                href="{routeString.serviceAccountList}/{serviceAccount.metadata
                  ?.namespace}/{serviceAccount.metadata?.name}"
                use:link
              >
                {serviceAccount.metadata?.name}
              </a>
            </td>
            <td>{serviceAccount.metadata?.namespace}</td>
          </tr>
        {/each}
      </tbody>
    </table>
