<script lang="ts">
  ...
  import type { V1RoleBindingList } from "@kubernetes/client-node/dist/gen/api";
...
<div class="router-page" in:fade|global={{ duration: 250 }}>
  ...
  {:else if roleBindingListData}
    <table class="table table-pin-rows">
      ...
      <tbody>
        {#each roleBindingListData.items as roleBinding}
          <tr>
            <td>
              <a
                class="hover:text-primary"
                href="{routeString.roleBindingList}/{roleBinding.metadata
            ...
            <td>{roleBinding.metadata?.namespace}</td>
          </tr>
        {/each}
      </tbody>
    </table>
