<script lang="ts">
  ...
  export let optsIcon: keyof typeof icons = "elipsisVertical";
...
<div
  class="dropdown dropdown-left dropdown-hover flex h-full items-center
    space-x-2 rounded-lg p-1 hover:bg-base-200 {optClassNames}"
>
  <SvgIcon type={optsIcon} classNames={"size-4 h-full"} />

  <div
    bind:this={el}
    class="dropdown-content z-[1] flex h-full place-items-center
      items-center pr-2 {elCount > 1 ? 'join' : ''}"
  >
